...
<div class="top-header">
    <h4><i class="bi bi-geo-alt"></i> تعديل الموقع — {{ $location->name }}</h4>
    <div class="d-flex gap-2">
        <a href="{{ route('locations.show', $location->id) }}" class="btn btn-edit">عرض</a>
    ...
    <form action="{{ route('locations.update', $location->id) }}" method="POST">
        ...
        <div class="row g-3 mb-4">
            ...
            <div class="col-md-6">
                ...
                <select name="region_id" class="form-select">
                    ...
                    @foreach($regions->whereNull('parent_id') as $region)
                        <option value="{{ $region->id }}" {{ old('region_id',$location->region_id)==$region->id?'selected':'' }}>{{ $region->name }}</option>
                        @foreach($regions->where('parent_id', $region->id) as $child)
                            <option value="{{ $child->id }}" {{ old('region_id',$location->region_id)==$child->id?'selected':'' }}>&nbsp;&nbsp;&nbsp;↳ {{ $child->name }}</option>
                        @endforeach
                    @endforeach
                </select>
            </div>
            ...
            <div class="col-md-4">
                ...
                <input type="number" name="capacity" class="form-control" value="{{ old('capacity',$location->capacity) }}" min="0">
            </div>
            <div class="col-md-4">
                <label class="form-label">الحالة</label>
                <select name="is_active" class="form-select">
                    <option value="1" {{ old('is_active',$location->is_active)?'selected':'' }}>نشط</option>
                    <option value="0" {{ !old('is_active',$location->is_active)?'selected':'' }}>غير نشط</option>
                </select>
            </div>
            <div class="col-md-4">
                <label class="form-label">خط العرض (lat)</label>
                <input type="number" step="any" name="lat" class="form-control" value="{{ old('lat',$location->lat) }}">
            </div>
            <div class="col-md-4">
                <label class="form-label">خط الطول (lng)</label>
                <input type="number" step="any" name="lng" class="form-control" value="{{ old('lng',$location->lng) }}">
            </div>
            <div class="col-12">
