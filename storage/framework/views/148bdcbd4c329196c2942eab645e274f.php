<?php $__env->startSection('title', 'رخص الجاهزية'); ?>
<?php $__env->startSection('content'); ?>

<div class="top-header">
    <h4><i class="bi bi-shield-check"></i> رخص الجاهزية</h4>
    <a href="<?php echo e(route('readiness-licenses.create')); ?>" class="btn btn-add"><i class="bi bi-plus-lg"></i> إصدار رخصة</a>
</div>
    ...
    <form method="GET" class="row g-2 align-items-end" style="padding:14px">
        ...
        <div class="col-md-3">
            <select name="status" class="form-select" style="font-size:13px">
                ...
                <option value="active"    <?php echo e(request('status')=='active'    ?'selected':''); ?>>ساري</option>
                <option value="expired"   <?php echo e(request('status')=='expired'   ?'selected':''); ?>>منتهي</option>
                <option value="withdrawn" <?php echo e(request('status')=='withdrawn' ?'selected':''); ?>>مسحوب</option>
            </select>
        </div>
        <div class="col-auto">
        ...
        <div class="col-auto"><a href="<?php echo e(route('readiness-licenses.index')); ?>" class="btn btn-back" style="font-size:13px">مسح</a></div>
        <?php endif; ?>
    </form>
</div>
...
<div class="card">
    <div class="card-body p-0">
        ...
        <table class="table mb-0">
            ...
            <tbody>
                <?php $__currentLoopData = $licenses; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $lic): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php
                ...
                <tr>
                    <td>
                        <div style="font-weight:600;font-size:13px"><?php echo e($lic->employee->name); ?></div>
                        <div style="font-size:11px;color:#9ca3af"><?php echo e($lic->employee->department); ?></div>
                    </td>
                    <td style="font-size:13px"><?php echo e($lic->issued_at->format('Y-m-d')); ?></td>
                    <td style="font-size:13px"><?php echo e($lic->expires_at?->format('Y-m-d') ?? 'دائمة'); ?></td>
                    <td style="font-size:13px"><?php echo e($lic->issuer->name); ?></td>
                    <td>
                        <span style="background:<?php echo e($lb[$lic->status]??'#f3f4f6'); ?>;color:<?php echo e($lc[$lic->status]??'#6b7280'); ?>;padding:3px 10px;border-radius:20px;font-size:11px;font-weight:600">
                            <?php echo e($lic->status_label); ?>

                        </span>
                    </td>
                    <td>
                        <div style="display:flex;gap:4px">
                            <a href="<?php echo e(route('readiness-licenses.show', $lic->id)); ?>" class="btn btn-edit" style="font-size:11px;padding:4px 8px"><i class="bi bi-eye"></i></a>
                            ...
                            <button class="btn btn-delete" style="font-size:11px;padding:4px 8px;background:#fee2e2;color:#dc2626;border:none"
                                    onclick="withdrawLicense(<?php echo e($lic->id); ?>)"><i class="bi bi-shield-x"></i></button>
                            <?php endif; ?>
                            <form action="<?php echo e(route('readiness-licenses.destroy', $lic->id)); ?>" method="POST" onsubmit="return confirm('حذف هذه الرخصة؟')">
        ...
        <div style="padding:14px 20px"><?php echo e($licenses->withQueryString()->links()); ?></div>
        <?php else: ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\course laravel\example-app\resources\views/readiness-licenses/index.blade.php ENDPATH**/ ?>